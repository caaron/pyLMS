...
class BlockCoherenceDTD:
    def __init__(self, fs=16000, block_size=1024, hop_size=512, threshold=0.6):
        self.fs = fs
    ...
    #    doPlots = False
        ...
        for start in range(0, min_len - self.block_size + 1, self.hop_size):
            ...
            # Detect double talk
            double_talk = nccScore > self.threshold
            log_DT[start:start+self.hop_size] = np.ones(self.hop_size) * (1 if double_talk else 0)
            cc_values[start:start+self.hop_size] = np.ones(self.hop_size) * nccScore
            time_axis.append(start / self.fs)

            # Print detection status
            print(f"Time: {start / self.fs:.3f} s | Coherence: {nccScore:.4f} | Double Talk: {'YES' if double_talk else 'NO'}")
    ...
    #                plt.plot(f,Cxy,label='Cxy')
        ...
        if len(cc_values) < len(far_end):
            cc_values = np.concatenate((cc_values,np.zeros(len(far_end) - len(cc_values))))
        self.plot_results(far_end,near_end,cc_values)
        return log_DT, cc_values

    def plot_results(self,far_end,near_end,cc):
        # Load logged results
...
dtd = BlockCoherenceDTD(fs=Fs1, block_size=1024, hop_size=512, threshold=0.6)
log_DT, coherence_values = dtd.detect(far_end, mic)

mic = (d + near_end)/2.0
dtd_echo = BlockCoherenceDTD(fs=Fs1, block_size=1024, hop_size=512, threshold=0.6)
